...
public abstract class EgovComAbstractDAO extends EgovAbstractMapper {
	...
	public Integer selectCount(String queryId, Object search) throws DataAccessException {
		Integer cnt = (Integer) selectOne(queryId, search);
	...
	public Integer selectCount(String queryId) throws DataAccessException {
		Integer cnt = (Integer) selectOne(queryId, null);
		if(cnt == null || cnt.equals("")){
			cnt=0;
		}
		return cnt;
	...
	/**
	 * SQL 조회 결과를 ResultHandler 를 이용해서 출력한다.
	 * ResultHandler 를 상속해 구현한 커스텀 핸들러의 handleResult() 메서드에 따라 실행된다.
	 *
	 * @param queryId - 리스트 조회 처리 SQL mapping 쿼리 ID
